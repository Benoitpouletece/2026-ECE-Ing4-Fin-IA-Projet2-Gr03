...
class MCMCSampler:
    ...
    Cette classe encapsule la configuration et l'exécution de l'inférence MCMC
    pour les modèles de volatilité stochastique.
    """
    ...
    def __init__(
        ...
        """
        Initialise l'échantillonneur MCMC.
        
        Paramètres
        ----------
        model : callable
            Modèle probabiliste NumPyro
        num_warmup : int
            Nombre d'itérations de warm-up (burn-in)
        num_samples : int
            Nombre d'échantillons à générer après le warm-up
        num_chains : int
            Nombre de chaînes MCMC indépendantes
        chain_method : str
            Méthode d'exécution des chaînes ('parallel', 'sequential', 'vectorized')
        progress_bar : bool
            Afficher une barre de progression
        jit_model_args : bool
            Compiler les arguments du modèle avec JIT
        target_accept_prob : float
            Probabilité d'acceptation cible pour NUTS
        max_tree_depth : int
            Profondeur maximale de l'arbre pour NUTS
        init_strategy : str
            Stratégie d'initialisation ('uniform', 'prior', 'adapt_diag')
        """
        self.model = model
    ...
    def _get_init_strategy(self):
        """
        Retourne la stratégie d'initialisation appropriée.
    ...
    def run(
        ...
        *args
            Arguments positionnels pour le modèle
        **kwargs
            Arguments nommés pour le modèle
        ...
        # Configuration de l'échantillonneur NUTS
        nuts_kernel = NUTS(
            self.model,
        ...
        # Récupération des échantillons groupés par chaîne pour les diagnostics
        self.samples = self.mcmc.get_samples(group_by_chain=True)
        
        # DEBUG: Vérifier la structure des échantillons
    ...
    def get_samples(self) -> Dict[str, jnp.ndarray]:
        """
        Retourne les échantillons MCMC.
    ...
    def get_posterior_summary(self) -> Dict[str, Dict[str, float]]:
        """
        Calcule un résumé de la distribution postérieure.
        
        Retourne
        -------
        summary : dict
            Dictionnaire contenant les statistiques postérieures
        """
        if self.mcmc is None:
            raise ValueError("Aucune inférence n'a été effectuée. Appelez run() d'abord.")
        
        # Utiliser la méthode print_summary de MCMC et capturer la sortie
        summary = {}
        ...
        for param_name, param_samples in samples.items():
            if param_name not in ['dW_S', 'dW_v', 'obs']:
                ...
                summary[param_name] = {
                    ...
                    'n_eff': None,  # Sera calculé avec ArviZ
                    'r_hat': None   # Sera calculé avec ArviZ
                }
        
        return summary
    
    def print_summary(self):
        """Affiche le résumé de l'inférence MCMC."""
        if self.mcmc is None:
            raise ValueError("Aucune inférence n'a été effectuée. Appelez run() d'abord.")
    ...
    def compare_with_true_params(
        self,
        true_params: Dict[str, float]
    ) -> Dict[str, Dict[str, Any]]:
        """
        Compare les paramètres estimés avec les vrais paramètres.
        
        Paramètres
        ----------
        true_params : dict
            Dictionnaire des vrais paramètres
    ...
    def print_comparison(self, true_params: Dict[str, float]):
        """
        Affiche la comparaison avec les vrais paramètres.
    ...
    def get_inference_data(self):
        """
        Retourne les données d'inférence au format ArviZ.
        
        Retourne
        -------
        idata : InferenceData
            Données d'inférence au format ArviZ
        """
        if self.mcmc is None:
            raise ValueError("Aucune inférence n'a été effectuée. Appelez run() d'abord.")
...
class MCMCSamplerConfig:
    ...
    def get_fast_config() -> Dict[str, Any]:
        """
        Retourne une configuration rapide (pour les tests).
    ...
    def get_high_quality_config() -> Dict[str, Any]:
        """
        Retourne une configuration haute qualité (plus d'échantillons).
