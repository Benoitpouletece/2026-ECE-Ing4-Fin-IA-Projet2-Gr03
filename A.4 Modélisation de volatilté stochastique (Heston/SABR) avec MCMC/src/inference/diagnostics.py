...
Ce module fournit des outils pour analyser la convergence des chaînes MCMC
et valider les résultats de l'inférence bayésienne.
"""
...
class MCMCDiagnostics:
    ...
    Cette classe fournit des méthodes pour calculer et visualiser
    les diagnostics de convergence (R-hat, ESS, trace plots, etc.).
    """
    
    def __init__(self, samples: Dict[str, jnp.ndarray]):
        """
        Initialise les diagnostics MCMC.
        
        Paramètres
        ----------
        samples : dict
            Dictionnaire contenant les échantillons MCMC
        """
    ...
    def compute_r_hat(self, param_name: str) -> float:
        """
        Calcule le R-hat (Gelman-Rubin statistic) pour un paramètre.
        
        Le R-hat compare la variance intra-chaîne et inter-chaîne.
        Une valeur < 1.1 indique une convergence acceptable.
    ...
    def compute_ess(self, param_name: str) -> float:
        """
        Calcule l'ESS (Effective Sample Size) pour un paramètre.
        
        L'ESS mesure le nombre d'échantillons indépendants équivalents
    ...
    def compute_ess_bulk(self, param_name: str) -> float:
        """
        Calcule l'ESS bulk (pour la moyenne) pour un paramètre.
        
        L'ESS bulk mesure l'efficacité de l'échantillonnage pour estimer
        la moyenne postérieure.
        ...
        # Si plusieurs chaînes, calculer l'ESS pour chaque chaîne et prendre le minimum
        if param_samples.ndim == 2:
            n_chains, n_samples = param_samples.shape
            ess_per_chain = []
    ...
    def compute_all_diagnostics(self) -> Dict[str, Dict[str, float]]:
        """
        Calcule tous les diagnostics pour tous les paramètres.
        
        Retourne
        -------
        diagnostics : dict
            Dictionnaire contenant les diagnostics pour chaque paramètre
        """
        for param_name in self.samples.keys():
            if param_name not in ['dW_S', 'dW_v', 'obs']:
    ...
    def print_diagnostics(self):
        """Affiche les diagnostics de convergence."""
    ...
    def plot_trace(self, param_names: Optional[List[str]] = None, 
                   figsize: tuple = (15, 10)):
        """
        Trace les trace plots pour les paramètres spécifiés.
        
        Paramètres
        ----------
        param_names : list, optional
            Liste des paramètres à tracer. Si None, tous les paramètres.
        figsize : tuple
            Taille de la figure
        """
        if param_names is None:
            param_names = [k for k in self.samples.keys() if k not in ['dW_S', 'dW_v', 'obs']]
            ...
            if param_name in self.samples:
                ...
                if param_samples.ndim == 2:
                    ...
                    for chain in range(n_chains):
                        axes[idx].plot(param_samples[chain, :], alpha=0.6, 
                                      linewidth=1, label=f'Chaîne {chain+1}')
                    axes[idx].legend()
                else:
    ...
    def plot_posterior(self, param_names: Optional[List[str]] = None,
                      figsize: tuple = (15, 10)):
        """
        Trace les distributions postérieures pour les paramètres spécifiés.
        ...
        for idx, param_name in enumerate(param_names):
            if param_name in self.samples:
                ...
                # Histogramme
                axes[idx].hist(param_samples, bins=50, density=True, 
                              alpha=0.7, edgecolor='black')
                
                # Moyenne et médiane
                axes[idx].axvline(x=np.mean(param_samples), color='r', 
                                 linestyle='--', label=f'Moyenne = {np.mean(param_samples):.4f}')
                axes[idx].axvline(x=np.median(param_samples), color='g', 
                                 linestyle='--', label=f'Médiane = {np.median(param_samples):.4f}')
                ...
                axes[idx].axvline(x=q97_5, color='b', linestyle=':', alpha=0.5,
                                 label=f'IC 95%: [{q2_5:.4f}, {q97_5:.4f}]')
    ...
    def plot_autocorrelation(self, param_names: Optional[List[str]] = None,
                            max_lag: int = 50, figsize: tuple = (15, 10)):
        """
        Trace l'autocorrélation pour les paramètres spécifiés.
    ...
    def check_convergence(self, r_hat_threshold: float = 1.1,
                         ess_threshold: float = 400) -> bool:
        """
        Vérifie si la convergence est acceptable.
        ...
        -------
        converged : bool
            True si la convergence est acceptable, False sinon
        """
        if not self.diagnostics:
            self.compute_all_diagnostics()
    ...
    def get_convergence_report(self) -> str:
        """
        Génère un rapport de convergence.
        ...
        report = "=" * 80 + "\n"
        report += "RAPPORT DE CONVERGENCE MCMC\n"
        report += "=" * 80 + "\n\n"
